...
const repository = {};
mappings.forEach(([ ext, scope, regexp ]) => repository[ext] = {
...
const header = [
    ...
    patterns : [
      {
        match : '\\d',
        name :
            [ scopes.header.value, scopes.header.contextLines.number ].join(' ')
      },
  ...
  },
  {
    ...
    name : scopes.header.meta,
    captures :
        {"1" : {name : scopes.header.key}, "2" : {name : scopes.header.value}}
  },
];
...
const plainText = [
  ...
  },
  {
    match : "^  ((\\d+)(:))|((\\d+)( ))(.*)",
    name :
        [ scopes.resultBlock.meta, scopes.resultBlock.result.meta ].join(' '),
    captures : {
      "1" : {
        name : [
...
const tmLanguage = {
  ...
  patterns : [
    ...header, ...mappings.map(([ ext ]) => ({include : `#${ext}`})),
    ...plainText
  ],
  repository
};
