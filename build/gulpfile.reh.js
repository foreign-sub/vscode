...
BUILD_TARGETS.forEach(({platform, arch}) => {
  ...
  gulp.task(task.define(`node-${platform}-${arch}`, () => {
    const nodePath =
        path.join('.build', 'node', `v${nodeVersion}`, `${platform}-${arch}`);
...
function nodejs(platform, arch) {
  ...
  if (arch === 'alpine') {
    const contents =
        cp.execSync(`docker run --rm node:${
                        nodeVersion}-alpine /bin/sh -c 'cat \`which node\`'`,
                    {maxBuffer : 100 * 1024 * 1024, encoding : 'buffer'});
    return es.readArray([ new File(
        {path : 'node', contents, stat : {mode : parseInt('755', 8)}}) ]);
  }
  ...
  return remote(`/dist/v${nodeVersion}/node-v${nodeVersion}-${platform}-${
                    arch}.tar.gz`,
                {base : 'https://nodejs.org'})
...
function mixinServer(watch) {
  ...
  function checkout() {
    ...
    exec(`git checkout ${
        packageJSON['distro']} -- src/vs/server resources/server`);
