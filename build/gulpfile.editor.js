...
let editorResources = ['out-editor-build/vs/base/browser/ui/codiconLabel/**/*.ttf'];
...
const languages = i18n.defaultLanguages.concat([]); // i18n.defaultLanguages.concat(process.env.VSCODE_QUALITY !== 'stable'
// ? i18n.extraLanguages : []);
...
const extractEditorSrcTask = task.define('extract-editor-src', () => {
	...
	standalone.extractEditor({
		...
		entryPoints: ['vs/editor/editor.main', 'vs/editor/editor.worker', 'vs/base/worker/workerMain'],
		inlineEntryPoints: [apiusages, extrausages],
		libs: [`lib.es5.d.ts`, `lib.dom.d.ts`, `lib.webworker.importscripts.d.ts`],
...
const compileEditorESMTask = task.define('compile-editor-esm', () => {
	...
	if (process.platform === 'win32') {
		...
		result = cp.spawnSync(`node`, [`../node_modules/.bin/tsc`], { cwd: path.join(__dirname, '../out-editor-esm') });
	}
	...
	if (result.status !== 0) {
		...
		return util
			...
			.then(() => {
				...
				// create the commit
				cp.spawnSync(`git`, [`commit`, `-m`, `"original sources"`, `--no-gpg-sign`], { cwd: destPath });
...
const finalEditorResourcesTask = task.define('final-editor-resources', () => {
	return es.merge(
		...
		// dev folder
		es.merge(gulp.src('out-editor/**/*')).pipe(gulp.dest('out-monaco-editor-core/dev')),
		...
		es
			...
			.pipe(
				es.through(function(data) {
					...
					let relativePathToMap = path.relative(
...
gulp.task(
	...
	task.series(
		...
		task.parallel(
			task.series(compileEditorAMDTask, optimizeEditorAMDTask, minifyEditorAMDTask),
