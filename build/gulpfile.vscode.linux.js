...
const BUILD_TARGETS = [{ arch: 'x64' }, { arch: 'arm' }, { arch: 'arm64' }];
...
BUILD_TARGETS.forEach(buildTarget => {
...

	{
		...
		const prepareDebTask = task.define(
			...
			task.series(util.rimraf(`.build/linux/deb/${debArch}`), prepareDebPackage(arch))
		);
...

	{
		...
		const prepareRpmTask = task.define(
			...
			task.series(util.rimraf(`.build/linux/rpm/${rpmArch}`), prepareRpmPackage(arch))
		);
...

	{
		const prepareSnapTask = task.define(
			...
			task.series(util.rimraf(`.build/linux/snap/${arch}`), prepareSnapPackage(arch))
		);
