...
const WEB_MAIN = path.join(APP_ROOT, 'src', 'vs', 'code', 'browser',
                           'workbench', 'workbench-dev.html');

const args = minimist(process.argv, {
  boolean : [ 'no-launch', 'help' ],
  string : [ 'scheme', 'host', 'port', 'local_port' ],
});
  ...
  try {
    if (pathname === '/favicon.ico') {
      ...
      return serveFile(req, res,
                       path.join(APP_ROOT, 'resources', 'win32', 'code.ico'));
    }
...
function handleStatic(req, res, parsedUrl) {
  ...
  const relativeFilePath = path.normalize(
      decodeURIComponent(parsedUrl.pathname.substr('/static/'.length)));
...
function handleStaticExtension(req, res, parsedUrl) {
  ...
  const relativeFilePath = path.normalize(decodeURIComponent(
      parsedUrl.pathname.substr('/static-extension/'.length)));
...
async function handleRoot(req, res) {
  ...
  await Promise.all(extensionFolders.map(async extensionFolder => {
    try {
      const packageJSON =
          JSON.parse((await util.promisify(fs.readFile)(path.join(
                          EXTENSIONS_ROOT, extensionFolder, 'package.json')))
                         .toString());
      ...
      mapExtensionFolderToExtensionPackageJSON.set(extensionFolder,
                                                   packageJSON);
    } catch (error) {
      return null;
    }
  ...
  // Built in extensions
  mapExtensionFolderToExtensionPackageJSON.forEach(
      (packageJSON, extensionFolder) => {
  ...
  const data =
      (await util.promisify(fs.readFile)(WEB_MAIN))
          .toString()
          .replace('{{WORKBENCH_WEB_CONFIGURATION}}',
                   escapeAttribute(JSON.stringify({
                     staticExtensions,
...
async function serveFile(req, res, filePath,
  ...
  try {
    ...
    // Headers
    responseHeaders['Content-Type'] = textMimeType[path.extname(filePath)] ||
